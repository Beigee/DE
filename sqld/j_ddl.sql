...
create table YSJ_EMP(
    ...
    -- 기본키는 최소성(NOT NULL)과 유일성이(UNIQUE) 만족되어야 한다.
    -- 최소성과 유일성이 만족되는 컬럼이 많은 경우
    -- 대표성을 지니는 컬럼을 기본키로 지정한다.
    -- 기본키를 지정하면 고유 인덱스가 자동으로 생성된다.
    ...
    --          NULL 대신 기본 값으로 설정한 값 입력
    HIRE_DATE DATE DEFAULT SYSDATE,
    
    -- 고유 무결성(UNIQUE)
    -- : 테이블의 특정 컬럼값에 대해서 각 ROW의 값들이 모두 서로 달라야 한다는 규정(중복값 X)
    ENO CHAR(14) UNIQUE,
    
    -- NULL 무결성 : 테이블의 특정 컬럼값이 NULL이 될 수 없음
    ENAME VARCHAR2(30 CHAR) NOT NULL,
    
    -- 도메인 무결성 : 특정 컬럼값이 그 컬럼에 정의된 도메인에 속한 값이어야 한다는 규정
    MARRIAGE CHAR(1) DEFAULT 'N' CHECK(MARRIAGE IN ('Y','N')),
    
    AGE NUMBER CHECK(AGE > 20)
    
    -- 외래키(참조키)
    -- 참조 무결성 : 기본키와 참조키(외래키) 간의 관계가 항상 유지 됨
    -- CONSTRAIN FOREIGH KEY(컬럼명) REFERENCES 부모테이블(컬럼명)
    -- ON UPDATE CASCADE : 부모테이블의 행이 수정되면 자식테이블의 행도 같이 수정(오라클 지원X)
    -- ON DELETE CASCADE : 부모테이블의 행이 삭제되면 자식테이블의 행도 같이 삭제
    --                  옵션 지정 안 하면 부모테이블 데이터 삭제 불가능
...
--3. 제약조건을 지정할 경우 해당 제약조건을 어긴 데이터가 존재 해서는 안된다.
-- NOT NULL, 단 NULL인 데이터가 단 하나도 없을 때만 지정 가능
-- UNIQUE, 단 중복 데이터가 없을 때만
...
-- 참조제약 조건을 삭제하면서 테이블을 삭제 하고 싶을 경우

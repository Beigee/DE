...
-- 작성방법 : select 컬럼명... from 테이블명 where 조건식

-- 1. job 테이블에서 직급 이름 조회
select job_name from job;

-- 2. employee 테이블에서 사번, 이름, 급여 조회
...
-- *** 리터럴 컬럼 ***
-- 구문에서 직접 사용되는 값을 표현할 때, select 절에서 사용할 때 마치 테이블에 존재하는 데이터 인 것 마냥 result set에 포함된다.
...
-- *** distinct ***
-- result set 에서 중복된 row를 제거 하고자 할 때
